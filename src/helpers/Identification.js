export const isLivestream = () => {
  ...
  const chatApp = document.querySelector('yt-live-chat-app') // This will also work for chat popout
...
// isChatPopout checks that page url matches format for live chat isChatPopout
export const isChatPopout = () => {
  if (window.location.href.match(/^http[s]{0,1}:\/\/www.youtube.com\/live_chat\?\S*&{0,1}v=[^&\s]*\S*$/)) return true
  return false
}
