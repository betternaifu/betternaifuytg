  ...
  handleOptionsPopup () {
    ...
    const makeOptionsPopup = () => {
      chrome.windows.getCurrent((window) => {
        const height = 590
        const width = 395
        const topVal = Math.round((window.height - height) * 0.2 + window.top)
        const leftVal = Math.round((window.width - width) * 0.99 + window.left)
  ...
  insertScripts () {
    ...
    chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
      if (message.name === 'disableEmojiComplete') {
        const disableAutoEmoji = `var script=document.createElement(\'script\');script.text="var renderer=document.querySelector(\'iframe#chatframe\')?.contentDocument.querySelector(\'yt-live-chat-text-input-field-renderer#input.yt-live-chat-message-input-renderer\')??document.querySelector(\'yt-live-chat-text-input-field-renderer#input.yt-live-chat-message-input-renderer\');var emotes=[\'${message.emotes.join("\',\'")}\'];var emojiShortcutMap = renderer.emojiManager.emojiShortcutMap;var emojiShortcuts = Object.keys(emojiShortcutMap);emotes.filter((key) => emojiShortcuts.includes(key.toLowerCase())).forEach((key) => delete emojiShortcutMap[key.toLowerCase()]);";script.type=\'text/javascript\';var node=document.querySelector(\'yt-live-chat-text-input-field-renderer\')??document.querySelector(\'iframe#chatframe\').contentDocument.querySelector(\'yt-live-chat-text-input-field-renderer\');node.appendChild(script);`
